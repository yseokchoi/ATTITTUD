...
def get_options():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="number of reruns so we can estimate confidence interval",
    )
    parser.add_argument(
        "-no-src-only", action="store_true", help="do not train the source only model"
    )
    parser.add_argument(
        "-no-tgt-only", action="store_true", help="do not train the target only model"
    )
    parser.add_argument(
        "-ft-model",
        default="none",
        type=str,
        choices=["src", "src_pca", "none"],
        help="model to finetune. src finetunes on model trained only on the source distribution. src_pca finetunes model that was trained on source distribution using ATTITTUD updates",
    )
    parser.add_argument(
        "-exp-name", default="test_run", type=str, help="name of this experiment"
    )
    parser.add_argument(
        "-eta-exp",
        action="store_true",
        help='Vary the projection coefficients eta. Thus using -eta-exp  "(1, 1, 0)|(1, -1, 1)" will run num_runs experiments for each of these 2 configurations of eta  ',
    )
    parser.add_argument(
        "-use-jvp",
        action="store_true",
        help="use jvp for finding the subspace components",
    )
    parser.add_argument(
        "-multi-cifar",
        action="store_true",
        help="do multicifar100 experiments on superclasses. Assumes num-runs is the number of multicifar100 classes in consideration",
    )
    parser.add_argument(
        "-data-seed", type=int, default=1234, help="The seed to use for the dataset"
    )
...
def run_model(
    ...
    dataset,
    save_fldr,
    proj_lambda_="(1.0, 1.0, 1.0)",
    pca_grad=False,
    trainset_id="src_train",
    valset_id="src_valid",
    use_tgt_labels=False,
    tgt_labels_only=False,
    viz=True,
    ft=False,
    set_out_classes=True,
    num_classes=10,
    is_tgt_only=False,
    use_last=False,
):
    ...
    trainer = Trainer(
        ...
        opts.chkpt_path,
        opts.patience,
        proj_lambda_=proj_lambda_,
        pca_grad=pca_grad,
        pca_layers=eval(opts.pca_layers),
        num_pca_basis=opts.num_pca_basis,
        pca_nsamples=opts.pca_nsamples,
        pca_every=opts.pca_every,
        do_multitask=do_multitask,
        min_tr_loss=opts.min_tr_loss,
        val_taskweight=opts.val_taskweight,
        lowrank_nsamples=opts.lowrank_nsamples,
        do_pcgrad=do_pcgrad,
        is_chexnet=is_chexnet,
    )
    ...
    metrics, best_val = trainer.train(
        ...
        valset_id=valset_id,
        use_tgt_labels=use_tgt_labels,
        tgt_labels_only=tgt_labels_only,
        model_chkpt_path=save_path,
        num_classes=num_classes,
        out_classes=eval(opts.tgt_dgts),
        normal_train_iters=opts.normal_train_iters,
        not_is_ft=not ft,
        is_tgt_only=is_tgt_only,
        use_last=use_last,
    )
    ...
    tgt_test_iter = dataset.get_iter(
        opts.tgt_batch_sz, type_="tgt_test", tgt_labels_only=tgt_labels_only
    )
...
def get_dataset(opts):
    ...
    if opts.dataset_type == "MNIST":
        dataset = MNIST(
            ...
            opts.test_perclass,
            shuffle=True,
            save_path=opts.dataset_path,
            shots=opts.shots,
            num_monitor=opts.num_monitor,
            val_taskweight=opts.val_taskweight,
        )
        ...
        dataset = ChexPert_n_Imagenet(
            opts.imgnet_per_class,
            opts.imgnet_n_classes,
            opts.train_perclass * num_tgt_classes,
            opts.num_monitor,
            imgnet_save_path=opts.tiny_imagenet_loc,
            chexpert_save_path=opts.chexpert_loc,
            shuffle=True,
            shots=opts.shots,
            val_taskweight=opts.val_taskweight,
        )
...
def print_results(run_dict):
    ...
    for k, v in run_dict.items():
        ...
        if avg.shape[-1] == 1:
            ...
            str_ = [
                "{:20} {:.3f} +/- {:.4f}".format(z, x, y)
                for x, y, z in zip(avg[0], ci95[0], MY_CHEXPERT_CLASSES)
            ]
            str_ = "\n".join(str_)
            print("[{} | Tgt]\n{}".format(k, str_))
    print("\n")
...
def main():
    ...
    if opts.multi_cifar:
        ...
        projs = (
            projs * opts.num_runs
        )  # We want to run num_run experiments for each value of eta.
        num_iters = len(projs)
        ...
        all_weight_min_pairs = [
            (float(w), float(m)) for w in task_weights for m in task_min_tr
        ] * opts.num_runs
    ...
    for seed in range(num_iters):
        ...
        if opts.multi_cifar:
            ...
            seed = seed // (
                len(projs) // opts.num_runs
            )  # Same seed for different proj_lambda_ so we have a fair comparison
        elif opts.do_multitask:
        ...
        if not opts.no_tgt_only:
            ...
            save_fldr = path.join(
                opts.chkpt_path,
                "{}/tgt_dist".format(superclass_key if opts.multi_cifar else opts.seed),
            )
            train_out = run_model(
                opts,
                tgt_model,
                dataset,
                save_fldr,
                trainset_id="tgt_valid",
                valset_id="tgt_test",
                use_tgt_labels=True,
                tgt_labels_only=False,
                set_out_classes=False,
                num_classes=tgt_layers[-1],
                is_tgt_only=True,
            )
        ...
        if not opts.no_src_only:
            ...
            if cuda_available:
                src_model.cuda()
            desc = "src_dist_vwgt_{}_mintr_{}_{}".format(
                opts.val_taskweight, opts.min_tr_loss, pcgrad_ext
            )
            save_fldr = path.join(
                opts.chkpt_path,
                "{}/{}".format(superclass_key if opts.multi_cifar else opts.seed, desc),
            )
            train_out = run_model(
                opts,
                src_model,
        ...
        if opts.pca_grad:
            ...
            src_model_pca = get_model(
                opts,
                num_classes,
                layers=eval(opts.layer_sizes),
                make_funct=opts.use_jvp,
                pca_layers=eval(opts.pca_layers),
            )
            if cuda_available:
                src_model_pca.cuda()
            desc = "{}/pca_{}_nc_{}_vwgt_{}_mintr_{}".format(
                superclass_key if opts.multi_cifar else opts.seed,
                opts.proj_lambda_,
                opts.num_pca_basis,
                opts.val_taskweight,
                opts.min_tr_loss,
            ...
            train_out = run_model(
                ...
                dataset,
                save_fldr,
                proj_lambda_=opts.proj_lambda_,
                pca_grad=opts.pca_grad,
                trainset_id="src_train",
                valset_id="tgt_valid",
                use_tgt_labels=False,
                tgt_labels_only=False,
            ...
            if len(eval(opts.tgt_dgts)) == 2:
                ...
                train_out = train_out if train_out > 0.5 else 1.0 - train_out
            run_dict[
                "Train-Source(PCA_{})|Evaluate-Target".format(opts.proj_lambda_)
            ].append(train_out)
        ...
        if ft_model is not None:
            ...
            ext_no_pca = "no_pca_vwgt_{}_mintr_{}_{}".format(
                opts.val_taskweight, opts.min_tr_loss, pcgrad_ext
            )
            ext = ext if opts.ft_model == "src_pca" else ext_no_pca
            save_fldr = path.join(
                opts.chkpt_path,
                "{}/finetune_src_{}_tgt".format(
                    superclass_key if opts.multi_cifar else opts.seed, ext
                ),
            )
            train_out = run_model(
                opts,
            ...
            run_dict[
                "Train-Source-{}|Finetune-Target|Evaluate-Target".format(ext)
            ].append(train_out)
            #########################################################################
            #########################################################################
